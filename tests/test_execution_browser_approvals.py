...
def test_coordinate_approval_body_renders_exact_trusted_review() -> None:
    ...
    assert "personal/checkout" in body
    assert "visual coordinate fallback" in body
    assert "sha256" not in body
    assert "transaction_commits" not in body
    assert f"```\n/approve {draft.id} {code}\n```" in body
    assert f"```\n/deny {draft.id} {code}\n```" in body
    assert "one-time code generated by Ricky" in body
    # Full exact bindings remain in the approval, not the conversational summary.
    assert challenge.approval.binding == binding
    assert isinstance(challenge.approval, ParkedBrowserTransaction)
    assert challenge.approval.coordinate == coordinate
...
async def test_readable_financial_review_preserves_fees_recurrence_and_commands(tmp_path):
    from ricky.browser.types import BrowserFinancialTransactionEnvelope
    from ricky.messaging.markdown import escape_markdown_text, split_message_text

    now = datetime.now(UTC)
    store = ExecutionStore(_settings(tmp_path))
    await store.initialize()
    request, _ = await _running(store, now)
    base = _draft(request, now)
    terms = ("Cancellation requires notice. " * 20).strip()
    envelope = BrowserFinancialTransactionEnvelope.model_validate(
        {
            "kind": "financial",
            "intent": "Start the reviewed subscription",
            "payee": "Merchant **proposal**",
            "total": {"amount": "21.60", "currency": "USD"},
            "components": (
                {"label": "Subscription", "amount": {"amount": "20.00", "currency": "USD"}},
            ),
            "fees": ({"label": "Service fee", "amount": {"amount": "1.60", "currency": "USD"}},),
            "timing": "recurring",
            "recurrence": {
                "amount": {"amount": "21.60", "currency": "USD"},
                "cadence": "monthly",
            ...
            "consequences": ("Charges every month until cancelled",),
            "expected_result": "Subscription confirmation",
        }
    )
    draft = BrowserTransactionApprovalDraft.model_validate(
        {
    ...
    code = "test-only-correlation-token"
    body = _browser_approval_body(
        BrowserTransactionChallenge(
            approval=issue_parked_approval(draft, code=code),
            code=code,
        )
    )
    assert "Total charge: **21.60 USD**" in body
    assert "Service fee: 1.60 USD" in body
    assert "Recurs: 21.60 USD monthly" in body
    assert "2026-10-01" in body and "no end date" in body
    assert escape_markdown_text(terms) in body
    assert r"Merchant \*\*proposal\*\*" in body
    chunks = split_message_text(body, text_format="portable_markdown_v1", limit=1_000)
    assert all(len(chunk) <= 1_000 for chunk in chunks)
    assert any(f"/approve {draft.id} {code}" in chunk for chunk in chunks)
    assert any(f"/deny {draft.id} {code}" in chunk for chunk in chunks)
