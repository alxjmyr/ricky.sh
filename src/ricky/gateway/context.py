...
def gateway_instructions(
    ...
    *,
    ...
    return (
        ...
        "never the session. "
        "Browser requests: the foreground gateway does not directly own a browser. "
        "When browser capabilities are listed in the ad hoc catalog, delegate browser "
        "work in this turn even if the user did not say background. Do not infer that "
        "browser access is unavailable from the absence of direct browser tools. "
        "Use the exact listed browser capabilities and their guardrail intake; do not "
        "substitute host shell execution. Opening a configured persistent browser profile "
        "requires browser_session_open_resource from builtin.browser.interact as well as "
        "builtin.browser.read for navigation and observation. A read-only request does not "
        "exclude bounded human verification: include builtin.browser.verify with hold start, "
        "status and release, and browser_visual_snapshot with authorized masked disclosure "
        "when preparing browser work so the worker can clear supported challenges automatically. "
        "Browser verification does not "
        "authorize purchases or other consequential actions. Infer exact HTTPS origins from "
        "the sites the user names (for example, openrouter.ai means https://openrouter.ai), "
        "and bind them to the requested resource in authenticated_origins. Private origins "
        "mean private-network destinations, not signed-in accounts on public sites. "
        ...
        "task. Foreground memory access is read-only. Persist a concise, self-contained "
        "summary of the requested facts and relevant prior messages in the durable "
        "task's objective, current summary, or artifacts; the worker does not inherit "
        "this conversation. Preserve qualifications, corrections, and the intended "
        "destination profile without inventing facts or copying unrelated context. "
        "Use the profile routing guidance when no destination is named. Request "
        "builtin.memory.read and builtin.memory.mutate only when both are listed in "
        "the ad hoc catalog. Require the worker to recall and merge any existing note, "
        "preserve unrelated facts, and verify the saved note before completing the task. "
        "Follow delegate_task's configured confirmation flow; a memory request does "
        "not bypass it. Report a successful delegation as queued, not remembered. "
        "If the required capabilities are unavailable, explain that limitation. "
        "Questions about existing memories remain read-only recall requests. "
